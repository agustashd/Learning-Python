...
with open('ventas.csv') as archivo:
    ...
    for l in archivo:
        ...
        n += 1
        if len(lista) != 5:
            raise Exception('La linea {} tiene un numero invalido de campos'.format(n))
        if lista[col1] == '':
            raise Exception('El campo CODIGO de la linea {}  esta vacio'.format(n))
        try:
            int(float(lista[col4]))
        except ValueError:
            raise Exception('El campo CANTIDAD de la linea {} no es un numero entero'.format(n))
        try:
            float(lista[col5])
        except ValueError:
            raise Exception('El campo PRECIO de la linea {} no es un valor decimal'.format(n))
